...
coho_smiles_parse(struct coho_smiles *x, const char *smiles, size_t sz)
{
    ...
    for (;;) {
        ...
        case INIT:
            if (eos) {
                strlcpy(x->error, "empty SMILES", sizeof(x->error));
                goto err;
            }
                ...
                strlcpy(x->error, "atom expected", sizeof(x->error));
                goto err;
            }
            state = ATOM_READ;
            break;
        ...
        case DOT_READ:
            ...
            if ((rc = atom_ringbond(x, &anum))) {
                ...
                strlcpy(x->error, "atom must follow dot", sizeof(x->error));
                goto err;
            }
            state = ATOM_READ;
            break;
        ...
        case BOND_READ:
            ...
            if ((rc = atom_ringbond(x, &anum))) {
                ...
                strlcpy(x->error, "atom must follow bond", sizeof(x->error));
                goto err;
            }
            state = ATOM_READ;
            break;
...
add_ringbond(struct coho_smiles *x, int rnum, struct coho_smiles_bond *b)
{
    ...
    if (rb->atom0 == b->atom0) {
        strlcpy(x->error, "atom ring-bonded to itself", sizeof(x->error));
    ...
    if (rb->order == COHO_SMILES_BOND_UNSPECIFIED)
        ...
        strlcpy(x->error, "conflicting ring bond orders", sizeof(x->error));
        x->error_position = x->atoms[b->atom0].position;
        return -1;
    }
    if (rb->order == COHO_SMILES_BOND_UNSPECIFIED)
...
ringbond(struct coho_smiles *x, int anum)
{
    ...
    if (t.type == PERCENT) {
        ...
        if (!match(x, &t, 0, DIGIT)) {
            strlcpy(x->error, "2 digit ring bond expected", sizeof(x->error));
            return -1;
        }
        rnum += t.intval;
    } else {
